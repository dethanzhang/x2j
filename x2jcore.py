...
class x2jcore:
    ...
    def storeErrorMsg(self,i,j):
        ...
        self.error_msg[self.current_sheet].append("第【%s】行 第【%s】列单元格填写错误"%(i+1,col_name))
        self.error_cnt += 1

    def readExcel(self): #返回0=导表成功 1=字段名带空格
    ...
    def readExcelByLevel(self,findLevel):
        table = []
        row_num = self.sheet_data.nrows
